...
ggplot() + geom_polygon(data = ggplot2::map_data("state")  %>% # united states data
  ...
  scale_fill_viridis_d(option = "inferno", direction = -1, drop = FALSE, 
                       name="Percent\nOccupancy",
                       labels=c("0-90%", "90-100%", "100-125%", "125-150%", "150-200%", "200-250%", "250-300%","300+%")) +
  ...
  geom_text() +
  annotate("text", label = paste0("Total prison occupancy: ", round(mean(as.numeric(unlist(data %>% filter(Year == set_year) %>% dplyr::select(Percent_occupancy_fix)), na.rm = T), digits = 5)) ,"%"), x = -117.0, y = 41.0, size = 4, colour = "black") + #add mean occupancy text
    theme_classic() 
